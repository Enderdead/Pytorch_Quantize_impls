...
def adjust_learning_rate(optimizer, epoch):
    ...
    lr = LEARNING_RATE * (1- 0.0005 * (epoch))
    print(lr)
...
for epoch in range(EPOCH):
    ...
    print("\t Accuracy with drop out: {}".format(accucacy/X_test.shape[0]))
    accucacy = 0.0
    for X, Y in valid_set:
        ...
        _, val = torch.max(outputs, 1)
        accucacy += np.sum((val == Y.long()).data.cpu().numpy())
    print("\t Accuracy without drop out: {}".format(accucacy/X_test.shape[0]))
