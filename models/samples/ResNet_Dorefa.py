...
from functions.dorefa_connect import *
from layers.dorefa_layers import DorefaConv2d
...
class PreActBlock_conv_Q(nn.Module):
  ...
  def __init__(self, wbit, abit, in_planes, out_planes, stride=1):
    ...
    self.act_q = nnDorefaQuant(bitwight=abit)

    self.bn0 = nn.BatchNorm2d(in_planes)
    self.conv0 = DorefaConv2d(in_planes, out_planes, weight_bit=wbit, kernel_size=3, stride=stride, padding=1, bias=False)
    self.bn1 = nn.BatchNorm2d(out_planes)
    self.conv1 = DorefaConv2d(out_planes, out_planes, weight_bit=wbit, kernel_size=3, stride=1, padding=1, bias=False)

    self.skip_conv = None
    if stride != 1:
      self.skip_conv = DorefaConv2d(in_planes, out_planes,weight_bit=wbit, kernel_size=1, stride=stride, padding=0, bias=False)
