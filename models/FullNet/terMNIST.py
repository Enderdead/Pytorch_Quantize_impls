...
from layers.terner_layers import LinearTer
...
class TerMNIST(torch.nn.Module):
    def __init__(self, in_features, out_features, num_units=2048):
        ...
        self.linear1 = LinearTer(in_features, num_units)
        self.norm1   = nn.BatchNorm1d(num_units, eps=1e-4, momentum=0.15)

        self.linear2 = LinearTer(num_units, num_units)
        self.norm2  = nn.BatchNorm1d(num_units, eps=1e-4, momentum=0.15)

        self.linear3 = LinearTer(num_units, num_units)
