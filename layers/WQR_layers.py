...
class LinearQuantLin(torch.nn.Module, QLayer):
    ...
    def convert(other, bottom=-1, top=1, size=5, kapa=0, beta=0):
        ...
        result =  LinearQuantLin(other.in_features, other.out_features, False if other.bias is None else True, bottom=bottom, top=top, size=size, kapa=kapa, beta=beta)
...
class LinearQuantLog(torch.nn.Module, QLayer):
    ...
    def convert(other, gamma=2, init=0.25, size=5, kapa=0, beta=0):
        if not isinstance(other, torch.nn.Linear):
            raise TypeError("Expected a torch.nn.Linear ! Receive:  {}".format(other.__class__))
        result = LinearQuantLog(other.in_features, other.out_features, False if other.bias is None else True, gamma=gamma, init=init, size=size, kapa=kapa, beta=beta)
        result.weight.data.copy_(other.weight.data)
        if not other.bias is None:
            result.bias.data.copy_(other.bias.data)
        return result
